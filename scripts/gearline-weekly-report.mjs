...
if (analytics) {
  ...
  for (const [key, label] of [["activeUsers", "アクティブユーザー"], ["sessions", "セッション"], ["views", "表示回数"], ["engagementRate", "エンゲージメント率"], ["averageEngagementTimeSeconds", "平均エンゲージメント時間"], ["outboundClicks", "外部リンククリック"]]) nonNegative(analytics[key] ?? 0, label);
  ...
  analyticsLines = [
    ...
    "",
    "| アクティブユーザー | セッション | 表示回数 | エンゲージメント率 | 平均エンゲージメント時間 | 外部リンククリック |",
    "|---:|---:|---:|---:|---:|---:|",
    `| ${analytics.activeUsers ?? 0} | ${analytics.sessions ?? 0} | ${analytics.views ?? 0} | ${rate}% | ${avgTime}秒 | ${analytics.outboundClicks ?? 0} |`,
    "",
